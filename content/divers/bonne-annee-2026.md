Bonne Année !

Je profite de cette nouvelle année pour vous adresser mes meilleurs vœux et faire un point, en quelques chiffres, sur mon blog en 2025. 

J'ai écrit cette année 46 articles ! J'écris beaucoup d'articles en avance. Au moment où j'écris ces lignes j'ai des articles jusqu'à Mai. 

# En bricolisme

## Arduino

En 2026 j'ai ressortie les arduinos, et j'ai fait quelque petit projet. 
Comme d'habitude presque aucun n'est arrivé à bout. 
Mais tout de même l'un d'entre eux est particulièrement abouti et me sert à suivre la consommation de la maison et les températures de mon pc sur mon bureau. 
Une version simplifiée nous sert bien dans la cuisine également.

[gallery w=150 h=200]
pictures/blog/recap-2025/compagnon-arduino.jpg
[/gallery]

## Bois & mécanique

Je n'ai pas beaucoup bricolé à l'extérieur. 
Hormis certaines choses récurrentes comme le changement de pièce d'usure sur les voitures. 

J'ai cependant fait de nouveau bac à fleur pour madame avec les restes d'une terrasse d'un de nos voisins.

[gallery w=300 h=200]
pictures/blog/recap-2025/bac-fleur.jpg
[/gallery]


Mais également à l'école de mon petit garçon, ils ont organisé une journée bricolage et à l'aide d'une autre terrasse d'un autre voisin et des palettes, avec deux autres parents nous avons construit 2 petites cuisines.

[gallery w=200 h=200]
pictures/blog/recap-2025/cuisine-ecole-1.jpg
[/gallery]

## Impréssion 3D

J'ai également ressortie l'imprimante 3D, et j'ai surtout fabriqué des boîtes !
Et il y a eu ce projet de [M720Q avec une RX-6400](2025/pc-gaming-m720q-rx6400.html). 
Et sur ce projet, j'ai telement galéré avec ma veille imprimante de presque 10 ans 
et j'ai fini par acheter une nouvelle imprimante, la [Créality HI avec son module CFS](https://store.creality.com/fr/products/creality-hi-combo). 
J'ai patienté jusqu'au black friday pour la trouver à 350 euros.

[gallery w=200 h=200]
pictures/blog/recap-2025/creality-hi.png
pictures/linux/m720q-rx6400/21.jpg
[/gallery]

Puis j'ai cherché un peu ce qu'il existait en projet communautaire open dans le monde de l'impression 3D. 
J'ai découvert le projet [gridfinity de Zack Freedman](https://www.youtube.com/c/ZackFreedman/about). 
Et depuis c'est la catastrophe, l'imprimante tourne presque tous les jours, des boites encore des boites !

[gallery w=200 h=200]
pictures/blog/recap-2025/gridfinity-1.jpg
pictures/blog/recap-2025/gridfinity-2.jpg
[/gallery]

Et j'ai même fini par proposer mes [premiers modèles](https://www.printables.com/@Shionn_3251014/models). 

# Niveau code

L'année a été assez calme en code. 

## Python

Je ne suis pas un dev python. Je m'y suis mis contre mon grès sur une petite commu discord, j'ai depuis quitté le projet. 
Mais il reste [quelques articles](tag/python.html) sur ce blog de ce passage. 

## Godo

J'ai découvert le moteur de jeu open source [Godot](https://godotengine.org/fr/). Vous en saurez plus très prochainement. 

[gallery w=350 h=200]
pictures/blog/recap-2025/tortuga2.png
[/gallery]

## Java

Comme toujours, je code pas mal de petits outils rien que pour moi qui sont déployés sur mes serveurs.

- un outil de suivi de peinture [PainterTracker](https://github.com/shionn/PainterTracker)
- un outil de scan de pack [HumbleBunbdle](https://github.com/shionn/HumbleBundleGamePack)
 
# Niveau peinture

Mon année de peinture à été relativement bonne. 
Je n'ai pas atteint complètement mes objectifs. 
Je souhaitais :
- Peindre tous mes gobelins d’AoS et Battle, il m’en reste 4. 
- Finir mes elfes noirs, il m’en reste 10.
- Je voulais peindre le Core Set de D&L Deuslair, Et j'ai fini la derniere figurine le 31 décembre au soir.

En chiffre cela donne :
- 130 [Gobelins pour AOS](tag/gloomspite-gitz.html)
- 94 [Gobelins pour BAttle](tag/gloomspite-gitz.html)
- 119 Figurine de [Dungeon and Lasers](tag/dungeon-and-laser.html)
- 33 petite chose diverse
- Pour un total de 376 figurines peintent cette année, soit plus d'une par jour !

[gallery w=500 h=200]
pictures/aos/gloomspite/squig-herd/01.jpg
pictures/dungeon-and-laser/deuslair/core/redoe/ancient-protectors/01.jpg
[/gallery]

# JDR

Nous avons fait 16 parties sur la campagne de [Whisperia](tag/whisperia.html).
J'attends toujours les vidéos pour finaliser les comptes-rendus.

[gallery w=430 h=240]
pictures/jdr/whisperia/149.jpg
[/gallery]

# Niveau Linux

## En desktop

Alors c'est l'histoire d'un type qui découvre HyprLand, qui trouve ça trop bien et que ça lui rappelle [Awesome-wm](https://awesomewm.org/). 
Mais malheureusement sa distrution de coeur, [Debian](https://www.debian.org/index.fr.html) n'intègre que de veille version bien buggé. 

Alors ce type il se fdis, je vais packagé moi-même et faire un repository pour [Hyprland sur Debian](https://github.com/shionn/hypr-debian). Il y passe des mois, et finalement quand enfin sont repo devient à peu près fonctionnel, ben hop debian se met à jour et intègre de nouveau package pour Hyprland. 

Bon au moins il a appris à faire des paquets pour debian. 

[gallery w=711 h=200]
pictures/blog/recap-2025/hyprland.png
[/gallery]


## En serveur

J'ai beaucoup travaillé sur mes serveurs. Je vous présenterai peut-être un jour mon HomeLab.

- Mis en place du [transcodage matériel avec une A310](2025/jellyfin-proxmox-hardware-encode.html)
- Mis en place une grappe de serveur ProxMox pour assurer une meilleure disponibilité des services importants. 
- Mis en place un backup rotatif à l'aide d'un autre M720Q
- De l'IA avec un passthrought d'une Nvidia (article courant 2026).

[gallery w=200 h=200]
pictures/linux/proxmox-jellyfin/05-resultat.png
[/gallery]




